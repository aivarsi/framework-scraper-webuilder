  ...
  jQuery('article.method:has(h3.public)').each(function(i) {
      ...
      if (jQuery(this).find('h4:contains("Returns")').length) {
        if (jQuery(this).find('h4:contains("Returns")').next('a').length) {
          funcres = jQuery(this).find('h4:contains("Returns")').next('a').text().replace(/[余/g, '').trim().normalize_spaces().replace(/^\\/, '');
        } else {
          funcres = jQuery(this).find('h4:contains("Returns")').get(0).nextSibling.nodeValue.replace(/[余/g, '').trim().normalize_spaces();
        }
      }
  ...
  jQuery('article.property:has(h3.public)').each(function(i) {
      ...
      if (jQuery(this).find('h4:contains("Type")').length) {
        if (jQuery(this).find('h4:contains("Type")').next('a').length) {
          fieldtype = jQuery(this).find('h4:contains("Type")').next('a').text().replace(/[余/g, '').trim().normalize_spaces().replace(/^\\/, '');
        } else {
          fieldtype = jQuery(this).find('h4:contains("Type")').get(0).nextSibling.nodeValue.replace(/[余/g, '').trim().normalize_spaces();
        }
      }
